    ...
    def __getitem__(self, index):
        ...
        for stri in instanceidx:
            ...
            if self.augment['camera'] > 0: # and 'scenario31' in camera_dir:
                camera_dir = re.sub('camera_data/', 'camera_data_aug/', camera_dir)#Regular substitution
                camera_dir = camera_dir[:-4] + '_' + str(self.augment['camera']) + '.jpg'
                add_fronts.append(camera_dir)
            else:
                add_fronts.append(self.dataframe['unit1_rgb_'+stri][index])#image
            #lidar data
            lidar_dir = self.dataframe['unit1_lidar_'+stri][index]#lidar
            ...
            if self.augment['radar'] > 0:
                ...
                radar_dir = re.sub('radar_data/', 'radar_data_ang/', radar_dir)#radar
        ...
        for i in range(self.seq_len):
            if self.augment['camera'] == 0:
                ...
                imgs = np.array(Image.open(self.root+add_fronts[i]).resize((256,256)))#Read image data 256*256*3 (enhanced only)
            #radar data
            radar_ang1 = np.load(self.root + add_radars[i])#Read radar data 256*256(angle)
            ...
            if self.add_velocity:
                ...
                data['radars'].append(torch.from_numpy(np.concatenate([radar_ang, radar_vel], 0)))#@2*256*256(Spell ang and vel together)
            else:
                data['radars'].append(torch.from_numpy(radar_ang))
                # radar_vel1 = np.load(self.root + add_radars[i].replace('ang', 'vel'))
                # # radar_vel = np.expand_dims(radar_vel1, 0)
                # data['radars'].append(torch.from_numpy(radar_vel1))
            #lidar data
            PT = np.asarray(o3d.io.read_point_cloud(self.root+add_lidars[i]).points)
            PT = lidar_to_histogram_features(PT, add_lidars[i],custom_FoV=self.custom_FoV_lidar)#1*256*256
            if self.flip:
...
def xy_from_latlong(lat_long):
    ...
    # The coordinates are converted, turning latitude and longitude into xy coordinates
